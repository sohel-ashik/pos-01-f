...
import ContextSellData from "../contexts/ContextSellData";
import stringMatching from "../helper/stringMatching";
import { clearAddingData, countingAllCart, deleteSingle, editSingle, retrivingLocalAddData, savingLocalAddData } from "../helper/workingWithLocalStorageSell";
...
export function DateInput(){
    const {date,setDate} = useContext(ContextSellData);
...
export function SupplierAdder(){
    ...
    const {supplier,setSupplier, supplierArr} = useContext(ContextSellData);
        ...
        <div className="dropdown w-full">
            ...
            <div onClick={()=>setOpen(true)} >
                <fieldset className={`border px-4 pb-1 rounded-md shadow-md transition-color  ${clicked}`}>
                    <legend className="text-md text-gray-800  mb-2">ক্রেতা</legend>
                    <input 
                        onChange={(e)=>
...
export function IndividualProductAdder({btnName = "পণ্যটি যোগ করুন", editBtn, index, pre={}}){
    ...
    const {productArr, setProductArr, cartDetails, setCartDetails, totalAddNow, setTotalAddNow} = useContext(ContextSellData);
...
function SingleCard({name,quantity,rate,index}){
    ...
    const {productArr, setCartDetails,totalAddNow,setTotalAddNow} = useContext(ContextSellData);
...
export function SavedCart(){
    ...
    const {cartDetails} = useContext(ContextSellData);
...
export function ReviewCart(){
    ...
    const {date, supplier,totalAddNow, setTotalAddNow, preDue, setCartDetails,setPreDue} = useContext(ContextSellData);
    const [paid,setPaid] = useState(0);
    const [clicked,setClicked] = useState('');

    return(
        <div className="flex flex-col mt-10 ">
                <div className="text-center font-semibold text-lg py-2">মোট বিক্রি হিসাব</div>
